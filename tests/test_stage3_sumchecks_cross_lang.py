...
from jolt_verifier import verify_spartan_outer_stage1, verify_stage2, verify_stage3  # stage orchestration (final source of truth)
...
class Stage3SumchecksCrossLangTests(unittest.TestCase):  # Python↔Rust full-proof parity: Stage1 → Stage2 → Stage3.
    def test_stage3_sumchecks_match_rust_oracle(self):
        kv = _parse_kv_lines(run_rust_oracle("stage3_sumchecks_blake2b", "").strip())
        ...
        shift_claims = [Fr(int(x)) for x in kv["stage3_shift_claims"].split(",") if x]
        self.assertEqual(len(shift_claims), len(shift_names))
        for name, v in zip(shift_names, shift_claims):
        ...
        instr_claims = [Fr(int(x)) for x in kv["stage3_instr_claims"].split(",") if x]
        self.assertEqual(len(instr_claims), len(instr_names))
        for name, v in zip(instr_names, instr_claims):
        ...
        regs_claims = [Fr(int(x)) for x in kv["stage3_regs_claims"].split(",") if x]
        self.assertEqual(len(regs_claims), len(regs_names))
        for name, v in zip(regs_names, regs_claims):
        ...
        for j in range(stage3_num_rounds):
            coeffs = [Fr(int(x)) for x in kv[f"stage3_sumcheck_poly_{j}"].split(",") if x]
            stage3_polys.append(CompressedUniPoly(coeffs))
        stage3_sumcheck_proof = SumcheckInstanceProof(stage3_polys)

        r3 = verify_stage3(stage3_sumcheck_proof, trace_len, acc, t)

        want_stage3_r_sumcheck = [int(Fr(int(x))) for x in kv["stage3_r_sumcheck"].split(",") if x]
        got_stage3_r_sumcheck = [int(x) for x in r3]
        self.assertEqual(got_stage3_r_sumcheck, want_stage3_r_sumcheck)
